...
app.post('/api/bot-config/test-gpt', isAuthenticated, async (req, res) => {
  try {
    ...
    const botConfig = await BotConfig.findByPk(configId);
    ...
    const response = await openai.chat.completions.create({
      model: botConfig.model || 'gpt-3.5-turbo',
      messages: [
        { role: "system", content: botConfig.prompt || 'Você é um assistente útil e amigável.' },
