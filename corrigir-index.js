...
fs.readFile(indexPath, 'utf8', (err, data) => {
  ...
  // Encontrar a função generateGPTResponse
  const funcaoRegex = /async function generateGPTResponse\([\s\S]*?\}/g;
  
  // Nova implementação da função
  const novaFuncao = `async function generateGPTResponse(prompt, message, model = 'gpt-3.5-turbo') {
  try {
  ...
  let novoConteudo = data.replace(funcaoRegex, novaFuncao);
  
  // Verificar se a substituição foi feita
  if (novoConteudo === data) {
    console.log('Função generateGPTResponse não encontrada. Tentando outra abordagem...');
    
    // Tentar encontrar a função com uma regex mais simples
    const funcaoSimples = /async function generateGPTResponse[\s\S]*?}/;
    novoConteudo = data.replace(funcaoSimples, novaFuncao);
    
    if (novoConteudo === data) {
      console.error('Não foi possível encontrar a função generateGPTResponse no arquivo.');
      return;
    }
  }
  ...
  fs.writeFile(backupPath, data, 'utf8', (err) => {
    ...
    fs.writeFile(indexPath, novoConteudo, 'utf8', (err) => {
      ...
      console.log('A função generateGPTResponse foi corrigida.');
