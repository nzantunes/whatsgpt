...
app.get('/get-qrcode', isAuthenticated, (req, res) => {
  try {
    ...
    // Verificar se o QR code já está disponível
    if (global.qrcode) {
      // Verificar se o QR code está expirado (5 minutos)
      const qrExpired = global.qrCodeGeneratedAt && 
                       (Date.now() - global.qrCodeGeneratedAt > 5 * 60 * 1000);
                       
      if (qrExpired) {
        console.log('QR code expirado, solicitando novo...');
...
io.on('connection', (socket) => {
  ...
  // Enviar QR code se estiver disponível
  if (global.qrcode) {
...
client.on('message', async (message) => {
  ...
  try {
    ...
    // Obter o userId associado à configuração ativa
    const userId = await getUserIdFromActiveConfig();
    ...
    try {
      ...
      console.log(`✅ Conversa salva no banco de dados para: ${message.from}`);
    } catch (dbError) {
